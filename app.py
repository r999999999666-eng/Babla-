...
# ==========================================
# 2. ЖЕЛЕЗНАЯ ОТПРАВКА МЕДИАИ СООБЩЕНИЙ
# ==========================================
...
def send_media_bulletproof(chat_id, file_id, caption=None, reply_markup=None):
    ...
    try:
        ...
        # Перехватываем любую ошибку Telegram 400 (Bad Request / DOCUMENT_INVALID)
        if e.error_code == 400:
            try:
...
def start(msg):
    ...
    # Полностью сбрасываем застрявшие обработчики ввода файлов/текста
    ...
    try:
        ...
        print(f"⚠️ Ошибка очистки сообщений при start: {e}")
