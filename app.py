...
def send_media_bulletproof(chat_id, file_id, caption=None, reply_markup=None):
    """Стандартная отправка фото без удаления"""
    if not file_id:
        if caption:
    ...
    try:
        return bot.send_photo(chat_id, file_id, caption=caption, parse_mode='HTML', reply_markup=reply_markup)
