...
def send_media_bulletproof(chat_id, file_id, caption=None, reply_markup=None):
    ...
    try:
        ...
        # Если Telegram сообщает, что файл невалиден
        if any(err in e.description for err in ["DOCUMENT_INVALID", "wrong file identifier", "Bad Request", "PHOTO_INVALID_DIMENSIONS"]):
            # Автоочистка нерабочего QR из БД
            try:
                ...
                print(f"Ошибка очистки битого QR из базы: {db_err}")
...
def run_bot():
    ...
    try:
        # Автоматический сброс конфликтов Webhook
