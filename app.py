...
def start(msg):
    ...
    if chat_id in payment_timers:
        try: payment_timers[chat_id].cancel()
        except: pass
...
def back_to_main(msg): start(msg)
...
def back_handler(msg): start(msg)
...
{EMOJI['clock']} <b>Оплатите и отправьте скриншот чека в течение 5 минут!</b>"""
    ...
    if user_id in payment_timers:
        try: payment_timers[user_id].cancel()
        except: pass
...
def get_check_photo(msg):
    ...
    send_msg(user_id, f"{EMOJI['check']} <b>ЗАЯВКА ПРИНЯТА!</b>\n\n🆔 {safe_html(account_id)}\n{EMOJI['money']} СУММА: {amount:.2f} сом\n\n{EMOJI['clock']} ОЖИДАЙТЕ ОБРАБОТКИ ОПЕРАТОРОМ...", reply_markup=main_menu(user_id))
    temp_data.pop(user_id, None)
...
def withdraw_get_code(msg):
    ...
    send_msg(user_id, f"{EMOJI['check']} Ваша заявка на вывод принята оператором! Ожидайте выплаты.", reply_markup=main_menu(user_id))
    temp_data.pop(user_id, None)
...
def broadcast_start(msg):
    send_msg(msg.chat.id, f"{EMOJI['broadcast']} <b>Отправьте сообщение для рассылки (текст или фото):</b>", reply_markup=back_menu())
...
def handle_admin_callbacks(call):
    ...
    if data.startswith('approve_'):
        ...
        with get_db() as conn:
            row = conn.execute('SELECT user_id, amount, account_id, timestamp FROM deposits WHERE id = ?', (dep_id,)).fetchone()
        if row:
            ...
            try:
                bot.send_message(user_id, f"{EMOJI['check']} <b>Ваш баланс успешно пополнен!</b>\n\n{EMOJI['money']} <b>Сумма:</b> {amount:.2f} сом\n<b>Счет:</b> {safe_html(account_id)}\n⏱️ <b>Обработано за:</b> {elapsed}s")
            except: pass
            try:
                bot.edit_message_caption(f"{EMOJI['check']} ЗАЯВКА #{dep_id} ОДОБРЕНА", call.message.chat.id, call.message.message_id)
            except: pass
        ...
        if row:
            ...
            try:
                bot.edit_message_caption(f"{EMOJI['cross']} ЗАЯВКА #{dep_id} ОТКЛОНЕНА", call.message.chat.id, call.message.message_id)
            except: pass
        ...
        try:
            bot.edit_message_caption(f"{EMOJI['check']} ЗАЯВКА НА ВЫВОД #{w_id} ВЫПОЛНЕНА", call.message.chat.id, call.message.message_id)
        except: pass
        ...
        if row:
            bot.answer_callback_query(call.id, "❌ Отклонено")
            try:
                bot.send_message(row[0], f"{EMOJI['cross']} Заявка на вывод #{w_id} отклонена. Поддержка: {SUPPORT}")
            except: pass
        try:
            bot.edit_message_caption(f"{EMOJI['cross']} ЗАЯВКА НА ВЫВОД #{w_id} ОТКЛОНЕНА", call.message.chat.id, call.message.message_id)
        except: pass

# ==================== WEBHOOK (ГЛАВНОЕ ДЛЯ RENDER) ====================
@app.route('/')
def home():
...
def set_webhook():
    if not WEBHOOK_URL:
        logger.warning("WEBHOOK_URL не задан! Бот будет работать только через polling (ненадёжно на Render)")
        return
