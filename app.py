...
def clear_user_messages(user_id):
    """Удаляет все ранее сохраненные сообщения пользователя"""
...
def send_media_bulletproof(chat_id, file_id, caption=None, reply_markup=None):
    ...
    try:
        ...
        if msg: save_msg(chat_id, msg.message_id)
        return msg
    except telebot.apihelper.ApiTelegramException as e:
        if "DOCUMENT_INVALID" in e.description or "wrong file identifier" in e.description:
            try:
                msg = bot.send_document(chat_id, file_id, caption=caption, parse_mode='HTML', reply_markup=reply_markup)
                if msg: save_msg(chat_id, msg.message_id)
                return msg
            except Exception as doc_err:
                print(f"❌ Битый file_id [{file_id}]: {doc_err}")
                if caption:
                    msg = bot.send_message(chat_id, caption, parse_mode='HTML', reply_markup=reply_markup)
                    if msg: save_msg(chat_id, msg.message_id)
                return None
        else:
            print(f"⚠️ Ошибка Telegram API [{file_id}]: {e}")
            return None
    except Exception as general_err:
        print(f"⚠️ Системная ошибка при отправке медиа: {general_err}")
        return None
...
def start(msg):
    ...
    if not active and msg.from_user.id not in get_admins() and msg.from_user.id != MAIN_ADMIN:
        send_msg(msg.chat.id, f"{EMOJI['off']} <b>Бот временно отключен на техническое обслуживание.</b>")
        return
...
{EMOJI['support']} Помощь: {SUPPORT}"""
    ...
    send_msg(msg.chat.id, welcome_text, reply_markup=main_menu(msg.from_user.id))
...
def health_command(msg):
    ...
    if msg.from_user.id in get_admins() or msg.from_user.id == MAIN_ADMIN:
        ...
        send_msg(msg.chat.id, status_text)
...
def get_amount(msg):
    ...
    if qr_file_id:
        ...
        send_msg(msg.chat.id, f"{EMOJI['qr']} QR-код временно обновляется.")
